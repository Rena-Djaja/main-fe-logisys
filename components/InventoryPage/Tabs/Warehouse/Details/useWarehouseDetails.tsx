...
const useWarehouseDetails = ({ id }: CommonFormProps) => {
  ...
  const { data: warehouseDetails, isValidating } = useCommonApi<
    CommonDetailsRequest,
    WarehouseProps
  >(InventoryAPI.GET_WAREHOUSE_DETAILS, { id: Number(id) }, { method: 'GET' })

  const { data: stockList, isValidating: isStockListValidating } = useCommonApi<
    ProductStockRequest,
    ProductStockProps[]
  >(
    InventoryAPI.GET_WAREHOUSE_INVENTORY,
    {
      id: Number(id),
