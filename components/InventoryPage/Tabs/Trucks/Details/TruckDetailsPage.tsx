...
import { Funnel, Truck, UserRound } from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuGroup,
...
import useTruckDetails from '@/components/InventoryPage/Tabs/Trucks/Details/useTruckDetails'
import CommonPageLoading from '@/components/shared/Loading/CommonPageLoading'

const TruckDetailsPage: FC<CommonFormProps> = (props) => {
  const {
    truckDetails,
    isValidating,
    filter,
  ...
  return (
    <div className="mt-8 w-full flex flex-col gap-10">
      <div className="w-full flex flex-col">
        <h1 className="font-semibold text-[2rem]">Truck Details</h1>
      </div>
      <div className="w-full flex flex-col gap-4 pl-4">
        <div className="w-full flex items-center gap-3">
          <Truck className="size-4.5" />
          <span className="font-semibold text-[0.875rem]">
            {truckDetails?.plate_number}
          </span>
        </div>
        <div className="w-full flex items-center gap-3">
          <UserRound className="size-4.5" />
          <span className="font-semibold text-[0.875rem]">
            {truckDetails?.salesman_name}
          </span>
        </div>
      </div>
